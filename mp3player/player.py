...
class MP3Player:
    def __init__(self) -> None:
        ...
        # Set initial window size to 1600x1200 to provide more space for transcription text
    ...
    def on_canvas_right_click(self, event: tk.Event) -> None:
        ...
        centisecs = round((clicked_time - total_seconds) * 100)
        ...
        self.marker_millisecond_entry.insert(0, f"{centisecs:02d}")
    ...
    def on_canvas_click_marker_select(self, event: tk.Event) -> None:
        ...
        if marker_clicked is not None:
            ...
            minutes = total_seconds // 60
            seconds = total_seconds % 60
            centisecs = round((clicked_marker["time"] - total_seconds) * 100)
    ...
    def on_segment_select(self, event: tk.Event) -> None:
        ...
        if selection:
            ...
            if segment:
                ...
                self.update_current_playing_segment_label()
    ...
    def update_current_playing_segment_label(self) -> None:
        """Update the label showing which segment is currently playing or paused."""
        if not hasattr(self, "current_playing_segment_label_var"):
            return
        
        is_playing = (hasattr(self.segment_player, "is_active") 
                      and self.segment_player.is_active 
                      and hasattr(self.segment_player, "segment_is_playing") 
                      and self.segment_player.segment_is_playing
                      and hasattr(self.segment_player, "selected_segments_to_play")
                      and self.segment_player.selected_segments_to_play
                      and hasattr(self.segment_player, "current_segment_index"))
        
        is_paused = (hasattr(self.segment_player, "is_active")
                     and self.segment_player.is_active
                     and hasattr(self.segment_player, "segment_paused_position")
                     and self.segment_player.segment_paused_position is not None
                     and hasattr(self.segment_player, "selected_segments_to_play")
                     and self.segment_player.selected_segments_to_play
                     and hasattr(self.segment_player, "current_segment_index"))
        
        if is_playing or is_paused:
            current_idx = self.segment_player.current_segment_index
            if current_idx < len(self.segment_player.selected_segments_to_play):
                current_segment = self.segment_player.selected_segments_to_play[current_idx]
                start_time = self.format_time(current_segment.start_time)
                end_time = self.format_time(current_segment.end_time)
                status = "Playing" if is_playing else "Paused"
                self.current_playing_segment_label_var.set(
                    f"{status}: Seg{current_segment.index + 1} [{start_time} - {end_time}]"
                )
                return
        
        self.current_playing_segment_label_var.set("No segment playing")
    ...
    def toggle_segment_play_pause(self) -> None:
        ...
        if hasattr(self, "segment_play_pause_button"):
            if self.segment_player.segment_is_playing:
                self.segment_play_pause_button.config(text="Pause Segment")
            else:
                self.segment_play_pause_button.config(text="Play Segment")

    def handle_space_key(self, event) -> None:
        """Handle space key press to toggle segment play when no input has focus."""
        focused_widget = self.root.focus_get()
        # Check if the focused widget is an input widget (Entry or Text)
        if not isinstance(focused_widget, (tk.Entry, tk.Text)):
            self.toggle_segment_play_pause()
    ...
    def update_segment_list(self) -> None:
        ...
        try:
            ...
            for i, segment in enumerate(segments):
                ...
                # Get the start and end marker names directly from sorted_markers using segment index
                # This avoids floating point comparison issues
                start_marker_name = sorted_markers[i]["name"] if i < len(sorted_markers) else "Unknown"
                end_marker_name = sorted_markers[i + 1]["name"] if (i + 1) < len(sorted_markers) else "Unknown"

                # Format the display text according to the requested format
                # Extract marker numbers from names like "Marker0" -> 0, "Marker1" -> 1
                start_num = ''.join(filter(str.isdigit, start_marker_name)) if start_marker_name != "Unknown" else ""
                end_num = ''.join(filter(str.isdigit, end_marker_name)) if end_marker_name != "Unknown" else ""
                display_text = f"Seg{i + 1}:M{start_num}-M{end_num}([{self.format_time(segment.start_time)} To {self.format_time(segment.end_time)}][{total_seconds:.2f}S])"
            ...
            self.update_segment_time_display()
            # Update the segment selector combobox
            self.update_segment_selector()
    ...
    def update_segment_selector(self) -> None:
        """Update the segment selector combobox with current segments."""
        try:
            segments = self.segment_manager.get_segments()
            segment_list = []

            for i, segment in enumerate(segments):
                segment_list.append(f"Seg{i + 1}")

            if hasattr(self, "segment_selector"):
                self.segment_selector["values"] = segment_list
                # Select the currently selected segment if any
                if hasattr(self, "current_segment_index") and self.current_segment_index is not None:
                    if 0 <= self.current_segment_index < len(segment_list):
                        self.segment_selector.current(self.current_segment_index)

                # If no segment is currently selected but segments exist, select the first one
                elif segment_list:
                    self.current_segment_index = 0
                    self.segment_selector.current(0)

        except Exception as e:
            logger.error(f"Error updating segment selector: {e}")

    def on_segment_selector_changed(self, event) -> None:
        """Handle segment selector combobox selection change."""
        try:
            selected_index = self.segment_selector.current()
            if selected_index >= 0:
                self.current_segment_index = selected_index
                self.on_segment_select_ui_only(selected_index)
                logger.info(f"Segment selector changed to segment {selected_index + 1}")
        except Exception as e:
            logger.error(f"Error handling segment selector change: {e}")

    def on_segment_select_ui_only(self, index: int) -> None:
        """Select a segment by its index for viewing content only - does NOT affect playback or listbox selection."""
        try:
            segments = self.segment_manager.get_segments()
            if 0 <= index < len(segments):
                segment = segments[index]

                # Update the segment time display
                self.update_segment_time_display()

                # Display the content for this segment
            ...
            logger.error(f"Error selecting segment by index for UI only: {e}")
    ...
    def validate_time_input(self, event: Optional[tk.Event] = None) -> None:
        ...
        if widget == self.jump_minute_entry or widget == self.marker_minute_entry:
            # Minutes can be anything, but keep it reasonable (though no strict limit needed)
            pass
        elif widget == self.jump_second_entry or widget == self.marker_second_entry:
            ...
            if new_text:
                ...
                if sec_val > 59:
                    ...
                    new_text = "0"
        elif widget == self.jump_millisecond_entry or widget == self.marker_millisecond_entry:
            # Limit centiseconds to 0-99
            if new_text:
                cs_val = int(new_text)
                if cs_val > 99:
                    new_text = "99"
        ...
        if current_text != new_text:
            ...
            # Update preview line for marker time fields
            if widget == self.marker_minute_entry or widget == self.marker_second_entry or widget == self.marker_millisecond_entry:
                self.on_marker_time_change()
    ...
    def get_current_time(self) -> None:
        ...
        # Calculate minutes, seconds, and centiseconds from current position
        total_seconds = int(self.current_pos)
        minutes = total_seconds // 60
        seconds = total_seconds % 60
        centisecs = round((self.current_pos - total_seconds) * 100)
    ...
    def get_seg_current_time(self) -> None:
        ...
        # Calculate absolute time using segment start + seg_current_pos
        if (
            hasattr(self, "segment_player")
            and self.segment_player.is_active
            and hasattr(self.segment_player, "seg_current_pos")
        ):
            absolute_time = (
                selected_segment.start_time + self.segment_player.seg_current_pos
            )
        else:
            absolute_time = selected_segment.start_time

        # Calculate minutes, seconds, and centiseconds from the absolute time
        total_seconds = int(absolute_time)
        minutes = total_seconds // 60
        seconds = total_seconds % 60
        centisecs = round((absolute_time - total_seconds) * 100)
    ...
    def load_file(self) -> None:
        ...
        if filename:
            ...
            self.jump_millisecond_entry.insert(0, f"{centisecs:02d}")
    ...
    def create_fixed_markers(self):
        ...
        # Remove any existing Marker0 or Marker500 to replace with correct positions
        self.marker_manager.markers = [
            m
            for m in self.marker_manager.markers
            if m["name"] != "Marker0" and m["name"] != "Marker500"
        ]
        ...
        marker100 = {"time": marker100_time, "name": "Marker500"}
    ...
    def update_time_display(self) -> None:
        ...
        self.update_segment_time_label_display()
    ...
    def update_segment_time_label_display(self) -> None:
        """Update the segment time label display showing the relative time within the segment."""
        if not hasattr(self, "segment_time_label"):
            return
        ...
        if (
            ...
            self.segment_time_label.config(
                text="Segment time: N/A (no segment selected)"
            )
            return

        selected_segment = self.segment_manager.get_segment_by_index(selected_idx)
        if not selected_segment:
            self.segment_time_label.config(text="Segment time: N/A (segment not found)")
            return

        if self.segment_player.is_active and self.segment_player.segment_is_playing:
            seg_time = self.segment_player.seg_current_pos
        elif (
            hasattr(self, "segment_player")
            and self.segment_player.is_active
            and self.segment_player.seg_current_pos > 0
        ):
            seg_time = self.segment_player.seg_current_pos
        else:
            seg_time = 0.0

        seg_time_str = self.format_time(seg_time)
        self.segment_time_label.config(text=f"Segment time: {seg_time_str}")
    ...
    def update_segment_absolute_time_display(self) -> None:
        ...
        # Update the segment time label display
        self.update_segment_time_label_display()
    ...
    def format_time(self, seconds: float) -> str:
        """Format time in seconds to MM:SS.xx format."""
        if seconds <= 0:
            return "00:00.00"
        mins = int(seconds // 60)
        secs = int(seconds % 60)
        centisecs = round((seconds - int(seconds)) * 100)
        return TIME_FORMAT.format(mins, secs, centisecs)
    ...
    def _populate_jump_entries(self, position: float) -> None:
        ...
        minutes = total_seconds // 60
        seconds = total_seconds % 60
        centisecs = round((position - total_seconds) * 100)
    ...
    def validate_preview_duration(self, event: Optional[tk.Event] = None) -> None:
        ...
        # Restore segment selection after focus out
        self._on_any_widget_focus_out(event)
    ...
    def preview_marker_time(self) -> None:
        ...
        # Cancel any existing preview timer to avoid overlapping timers
        if hasattr(self, "_preview_timer_id") and self._preview_timer_id:
            self.root.after_cancel(self._preview_timer_id)
            self._preview_timer_id = None
            logger.info("Cancelled existing preview timer")

        try:
            minute_str = self.marker_minute_entry.get().strip()
            second_str = self.marker_second_entry.get().strip()
            centisecond_str = self.marker_millisecond_entry.get().strip()

            minutes = int(minute_str) if minute_str else 0
            seconds = int(second_str) if second_str else 0
            centiseconds = int(centisecond_str) if centisecond_str else 0

            preview_time = minutes * 60 + seconds + centiseconds / 100.0

            duration_str = self.preview_duration_entry.get().strip()
            logger.info(f"Preview duration entry value: '{duration_str}'")
            ...
            logger.info(f"Using preview_duration: {preview_duration}")
            ...
            self._preview_timer_id = self.root.after(preview_duration * 1000, stop_preview_and_return)
    ...
    def update_segment_time_display(self) -> None:
        ...
        for i in range(5):
            ...
            centisecs = round((relative_time_at_marker - total_seconds) * 100)
            time_text = f"{total_seconds}.{centisecs:02d}"
        ...
        if selected_segment.duration > 0:
            ...
            centisecs_current = int(
                (current_segment_time - total_seconds_current) * 100
            )
            current_time_text = f"{total_seconds_current}.{centisecs_current:02d}"

            anchor = "w"
            text_x = indicator_x + 5
    ...
    def transcribe_segment(self) -> None:
        ...
        if not tkmsg.askyesno(
            ...
            f"Do you want to transcribe segment {selected_idx + 1}?\n"
            f"Duration: {duration:.2f}(s)\n"
            f"Estimated time: ~{duration * 2:.0f}-{duration * 3:.0f} seconds"
        ):
            return
    ...
    def export_segments_to_lrc(self) -> None:
        """Export segments and their content to a .lrc file."""
        if not self.current_file:
        ...
        if not segments:
            ...
            try:
                with open(lrc_file, "w", encoding="utf-8") as f:
                    pass
                self.status_label.config(text="Created empty .lrc file (no segments)")
            except Exception as e:
        ...
        try:
            with open(lrc_file, "w", encoding="utf-8") as f:
                for segment in segments:
                    ...
                    secs = segment.start_time % 60
                    timestamp = f"[{mins:02d}:{secs:05.2f}]"
                    content = segment.content if segment.content else "[Instrumental]"
                    f.write(f"{timestamp}{content}\n")

            logger.info(f"Exported {len(segments)} segments to {lrc_file}")
            self.status_label.config(text=f"Exported: {os.path.basename(lrc_file)}")
        except Exception as e:
    ...
    def _on_any_widget_focus_in(self, event: Optional[tk.Event] = None) -> None:
        ...
        if hasattr(self, "segment_listbox"):
            selection = self.segment_listbox.curselection()
            if selection:
                self._saved_segment_selection = selection
                logger.debug(f"Saved segment selection: {selection}")

    def _on_any_widget_focus_out(self, event) -> None:
        """Restore segment listbox selection when any widget loses focus."""
        if hasattr(self, "segment_listbox") and hasattr(
            self, "_saved_segment_selection"
        ):
            saved_selection = self._saved_segment_selection
            self.segment_listbox.selection_clear(0, tk.END)
            for idx in saved_selection:
                if idx < self.segment_listbox.size():
                    self.segment_listbox.selection_set(idx)
            logger.debug(f"Restored segment selection: {saved_selection}")

    def _on_transcription_focus_in(self, event) -> None:
        """Save segment listbox selection when transcription text gets focus."""
        self._on_any_widget_focus_in(event)

    def _on_transcription_focus_out(self, event) -> None:
        """Restore segment listbox selection when transcription text loses focus."""
        self._on_any_widget_focus_out(event)
